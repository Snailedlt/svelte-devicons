...
(async () => {
  ...
  if (!deviconVersion.includes(currentVersion)) {
    ...
    await exec("zip svelte-devicons.zip dist -r");
    await axios({
      url: `https://uploads.github.com/repos/snailedlt/svelte-devicons/releases/${release_id}/assets?name=svelte-devicons.zip`,
      data: Buffer.from(await fsAsync.readFile("./svelte-devicons.zip")),
      headers: {
